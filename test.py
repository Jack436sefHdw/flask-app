...
def start_playcard():
    global image ,first_card, second_card, playcard_buttons, card_values, matched_pairs, flip_count, start_time
    playcard_buttons = []
    first_card = None
    second_card = None
    matched_pairs = 0
    ...
    lab2_1.config(text="計時配對時間:0:00")
    lab2_2.config(text="計算翻牌次數:0")
    clear_frame(fr2_2)
    clear_frame(fr2_3)
    selection = var.get()
    #print(selection)
    ...
    if selection == "52張":
        playcard_values = list(range(1, 14)) * 4
        random.shuffle(playcard_values)
        for i in range(5):
            for l in range(10):
                button = Button(fr2_2, image=image, command=lambda b=len(playcard_buttons): flip_card(playcard_buttons[b], b))
                button.grid(row=i, column=l,padx=1, pady=2)
                playcard_buttons.append(button)
        for k in range(2):
            button = Button(fr2_3, image=image, command=lambda b=len(playcard_buttons): flip_card(playcard_buttons[b], b))
            button.grid(row=0, column=k,padx=1, pady=2)
            playcard_buttons.append(button)
    else:
        playcard_values = list(range(1, 14)) * 4
        playcard_values += [0, 0]
