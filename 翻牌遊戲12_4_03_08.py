...
def start_playcard():
    ...
    if selection == "52張":
        for i in range(5):
            for l in range(10):
                button = Button(fr2_2, image=image, command=lambda b=len(buttons): flip_card(playcard_buttons[b], b))
                button.grid(row=i, column=l,padx=1, pady=2)
                playcard_buttons.append(button)
        for k in range(2):
            button = Button(fr2_3, image=image, command=lambda b=len(buttons): flip_card(playcard_buttons[b], b))
