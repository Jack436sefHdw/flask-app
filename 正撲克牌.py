...
def start_playcard():
    ...
    card_values = random.sample(
        [f"{suit}{number}" for suit in ["心", "桃", "菱", "梅"] for number in range(1, 14)] + ["醜0", "丑0"],
        54
    )
    ...
    for i in range(5):
        for j in range(10):
            idx = i * 10 + j
            btn = Button(fr2, image=back_image, command=lambda b=idx: flip_playcard(buttons[b], b))
    ...
    for k in range(4):  # 新增的卡片顯示在第六行
        idx = 50 + k
        btn = Button(fr3, image=back_image, command=lambda b=idx: flip_playcard(buttons[b], b))
...
start_btn = Button(frame1, text="開始", command=start_playcard)
